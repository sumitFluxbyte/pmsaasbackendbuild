...
export const addOrganisationMember = async (req, res) => {
    ...
    if (!user) {
        ...
        const adminName = newUserOrg?.organisation?.createdByUser.firstName &&
...
export const removeOrganisationMember = async (req, res) => {
    ...
    if (findAssignedProject.length > 0) {
        throw new BadRequestError("This user has active tasks or projects!");
    }
...
export const assignProjectAndRoleToUser = async (req, res) => {
    ...
    const projectsWithExistingPMs = projectsWithPMs
        .filter((pm) => arrayOfRoleAndProjectId.some((item) => item.projectId === pm.projectId &&
    ...
    if (projectsWithExistingPMs.length > 0) {
        throw new BadRequestError(`Project Manager already exists!! ${projectsWithExistingPMs}`);
    }
