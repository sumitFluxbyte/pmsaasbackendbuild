...
import { ProjectStatusEnum, TaskStatusEnum, UserRoleEnum, UserStatusEnum, } from "@prisma/client";
...
export const projectDashboardByprojectId = async (req, res) => {
    ...
    const reCalculateBudget = cpi !== 0 ? Number(projectWithTasks.estimatedBudget) / (cpi / 100) : 0;
    const budgetVariation = cpi !== 0
        ? Number(reCalculateBudget) - Number(projectWithTasks.estimatedBudget)
        : null;
    const reCalculatedDuration = spi !== 0 && estimatedDuration ? (estimatedDuration / spi) * 100 : 0;
