...
export const signUp = async (req, res) => {
    ...
    const { firstName, lastName, email, password, language } = authSignUpSchema.parse(req.body);
...
export const checkIp = async (req, res) => {
    ...
    if (ip) {
        const geo = lookup(ip);
        return new SuccessResponse(StatusCodes.OK, { ...geo, ip: ip }, "Ip fetch successfully").send(res);
    }
    else {
