...
function returnEndDate(nonWorkingDays, durationFromCurrentDate) {
    const currentDate = new Date();
    let endDate = new Date(currentDate.getTime());
    if (durationFromCurrentDate !== null &&
                    ...
                    const taskCreationPromises = project.tasks.map(async (task) => {
                        ...
                        const createParentTask = await prisma.task.create({
                            data: {
                                ...
                                kanbanColumnId: createProject.kanbanColumns.find((back) => back.name == "Backlog")?.kanbanColumnId,
                            },
                            include: {
                                parent: true,
                                project: true,
                        ...
                        if (createParentTask.parent?.taskId) {
                            await calculateDurationAndPercentage(createParentTask.parent.taskId, tenantId, organisationId);
                        }
                        const maxEndDateOne = await calculateProjectEndDate(createParentTask.projectId, tenantId, organisationId);
                        if (maxEndDateOne) {
                        ...
                        if (createParentTask && task.subTask) {
                            await Promise.all(task.subTask.map(async (sub) => {
                                const startDateTwo = returnEndDate(nonWorkingDays, sub.startDate);
                                ...
                                if (taskCreatedSub.parent?.taskId) {
                                    await calculateDurationAndPercentage(taskCreatedSub.parent.taskId, tenantId, organisationId);
                                }
                                const maxEndDate = await calculateProjectEndDate(taskCreatedSub.projectId, tenantId, organisationId);
                                if (maxEndDate) {
                                    await prisma.project.update({
                                        where: {
                                ...
                                if (sub.dependencies && sub.dependencies?.length) {
                                    ...
                                    for (const taskNumber of sub.dependencies) {
                                        if (isTaskNumber(taskNumber)) {
                                            ...
                                            if (findTask) {
                                                const [addDependencies1, addDependencies2] = await addDependenciesHelper(taskCreatedSub.taskId, findTask.taskId, tenantId, organisationId, createdByUserId, TaskDependenciesEnum.PREDECESSORS);
                                            }
                                        }
                                    }
                                }
