...
export const calculateEndDateFromStartDateAndDuration = async (startDate, duration, tenantId, organisationId) => {
    ...
    const nonWorkingDaysSet = new Set(orgDetails?.nonWorkingDays ?? []);
    const holidaysSet = new Set(orgDetails?.orgHolidays.map((holiday) => new Date(holiday.holidayStartDate).setUTCHours(0, 0, 0, 0)) ?? []);
    ...
    if (!nonWorkingDaysSet.has(startDayAbbreviation) &&
    ...
    while (remainingDuration > 0) {
        ...
        if (!nonWorkingDaysSet.has(dayAbbreviation) &&
            !holidaysSet.has(endDate.getTime())) {
            remainingDuration--;
        }
    }
