...
export async function calculationSPI(tenantId, organisationId, projectId) {
    ...
    for (const task of findTask) {
        ...
        const completionPercentage = Math.round(Number(await calculationSubTaskProgression(task, tenantId, organisationId))) ?? 0;
        ...
        while (currentDay <= effectiveDate) {
            ...
            if (nonWorkingDays.includes(dayAbbreviation)) {
                ...
                const isHolidayCounted = nonWorkingDays.includes(dayAbbreviation) && isNonWorkingDay;
        ...
        const dayDiffNew = dayOfEffectiveDate - dayOfStartDate - nonWorkingDaysCount + 1;
