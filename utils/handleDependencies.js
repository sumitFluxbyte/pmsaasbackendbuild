...
export const addDependenciesHelper = async (taskId, dependendentOnTaskId, tenantId, organisationId, userId, dependentType) => {
    ...
    if (dependentType == TaskDependenciesEnum.SUCCESSORS &&
        ...
        throw new BadRequestError("You're unable to designate an ongoing or completed task as a successor in an end to start dependency.");
    }
    ...
    const endDatePredecesssor = await taskEndDate(dependencyOnTask, tenantId, organisationId);
    const endDateDependentTask = await getNextWorkingDay(new Date(endDatePredecesssor), tenantId, organisationId);
    const getMaxEndDateOfSuccessforDependentTask = async (taskId) => {
        let maxdate;
        ...
        const existedDependenciesEndDates = await Promise.all(allDependenciesExisted.map((task1) => {
    ...
    const endDateSuccessorDependentTask = await getNextWorkingDay(new Date(await getMaxEndDateOfSuccessforDependentTask(latestTask.taskId)), tenantId, organisationId);
    ...
    if (dependentType == TaskDependenciesEnum.PREDECESSORS) {
        ...
        addDependencies1 = value1;
        addDependencies2 = value2;
        await helper(dependendentOnTaskId, tenantId, organisationId, userId, endDateSuccessorDependentTask);
    }
    if (dependentType == TaskDependenciesEnum.SUCCESSORS) {
        ...
        const getMaxEndDate = async () => {
            ...
            const allDependenciesExisted = await prisma.task.findMany({
                where: {
                    taskId: {
                        in: dependantTask.dependencies.map((dep) => dep.dependentOnTask.taskId),
                    },
                    dependencies: {
                        some: {
                            dependentType: TaskDependenciesEnum.SUCCESSORS,
            ...
            const existedDependenciesEndDates = await Promise.all(allDependenciesExisted.map((task) => {
...
export const handleSubTaskUpdation = async (tenantId, organisationId, taskId, completionPecentage, currentTaskId, startDate) => {
    ...
    if (hasDependencyError) {
        throw new BadRequestError(`You can not change the progress percentage for this task since it has an incompleted predecessors ${dependencyTaskNames.join(", ")}. Would you like to remove these dependencies?`);
    }
    if (hasDateError) {
        ...
        for (let obj of task.dependencies) {
            if (obj.dependentType === TaskDependenciesEnum.PREDECESSORS) {
                if (obj.dependendentOnTaskId === givenTaskId) {
                    throw new BadRequestError("You can not create this dependency as it would lead to an endless loop. If B is successor of A, then A can not be successor of B");
                }
                else {
...
export const detectCycleChild = async (taskId, tenantId, givenId) => {
    const prisma = await getClientByTenantId(tenantId);
    const checkSubtasksForCycle = async (task) => {
        if (task.subtasks && task.subtasks.length > 0) {
            for (let sub of task.subtasks) {
                if (sub.taskId === givenId) {
                    throw new BadRequestError("You're unable to create this dependency as it would lead to an endless loop.");
                }
...
export const helper = async (taskId, tenantId, organisationId, userId, newStartDate, canUpdateChild = true) => {
    ...
    if (task.parent?.taskId) {
        ...
        await helper(task.parent.taskId, tenantId, organisationId, userId, new Date(), false);
    }
};
