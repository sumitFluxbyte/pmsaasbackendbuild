...
export const getProjects = async (
  ...
  for (const project of projects) {
    ...
    const estimatedDuration = project.estimatedEndDate
      ...
      : null;
    const actualEndDate =
      project.tasks.length === 0 ? null : project.actualEndDate;
...
export const getProjectById = async (
  ...
  const actualEndDate =
    projects.tasks.length === 0 ? null : projects.actualEndDate;
  const response = {
    ...projects,
    progressionPercentage,
    actualDuration,
...
export const statusChangeProject = async (
  req: express.Request,
  res: express.Response
) => {
  if (!req.organisationId) {
...
export const addConsumedBudgetToProject = async (
  req: express.Request,
  res: express.Response
) => {
  if (!req.userId) {
  ...
  return new SuccessResponse(
    StatusCodes.OK,
    projectUpdate,
    "consumed budget updated successfully"
  ).send(res);
};
...
export const duplicateProjectAndAllItsTask = async (
  ...
  const duplicatedProjectName = await generateUniqueProjectName(
    project.projectName
  );
...
export const userAssignIntoProject = async (
  req: express.Request,
  res: express.Response
) => {
  if (!req.userId) {
  ...
  return new SuccessResponse(
    StatusCodes.OK,
    null,
    "Project assgined successfully."
  ).send(res);
};

export const updateProjectJobTitle = async (
  req: express.Request,
  res: express.Response
) => {
  const projectAssignUsersId = uuidSchema.parse(
