...
import {
  getNextWorkingDay,
  taskEndDate,
} from "../utils/calcualteTaskEndDate.js";
...
import {
  ...
  checkTaskStatus,
  deleteSubtasks,
} from "../utils/taskRecursion.js";
import { generateOTP } from "../utils/otpHelper.js";
import {
  attachmentAddOrRemove,
  commentEditorDelete,
  dependenciesAddOrRemove,
  taskUpdateOrDelete,
} from "../middleware/role.middleware.js";
import { enumToString } from "../utils/enumToString.js";
import {
  addDependenciesHelper,
  handleSubTaskUpdation,
  helper,
} from "../utils/handleDependencies.js";
...
export const getTasks = async (req: express.Request, res: express.Response) => {
  ...
  const finalArray = await Promise.all(
    tasks.map(async (task) => {
      ...
      const { flag, delay } = await taskFlag(
        task,
        req.tenantId,
        organisationId
      );
...
export const createTask = async (
  ...
  for (const [fieldName, fieldValue] of Object.entries(
    ...
    if (!excludedFields.includes(fieldName)) {
      if (fieldValue !== undefined && fieldValue !== null) {
        if (
          !(fieldName === "duration" && (fieldValue === 0 || fieldValue === 1))
        ) {
  ...
  const statusHandle = await checkTaskStatus(
    task.taskId,
    req.tenantId,
    req.organisationId
  );
...
export const updateTask = async (
  ...
  const prisma = await getClientByTenantId(req.tenantId);
  const { hasAccessIf, findtask } = await taskUpdateOrDelete(
    taskId,
    req.role!,
    req.userId,
    req.tenantId
  ...
  if (
    ...
    await handleSubTaskUpdation(
      ...
      req.organisationId,
      findtask.taskId,
      taskUpdateValue.completionPecentage ? true : false,
      taskId,
      taskUpdateValue.startDate
    );
  }
  ...
  const endDateNew = await taskEndDate(
    taskUpdateDB,
    req.tenantId,
    req.organisationId
  );
      ...
      try {
        if (dependantTask.dependentType === TaskDependenciesEnum.SUCCESSORS) {
          ...
          const getMaxEndDate = async () => {
            ...
            //   })
            // );
            const latestEndDate = await taskEndDate(
              taskUpdateDB,
              req.tenantId,
              organisationId
            );
          ...
          await helper(
            dependantTask.dependendentOnTaskId,
            req.tenantId,
            req.organisationId,
            req.userId,
            new Date(predecessorEndDateDependentTask)
          );
        }
      } catch (error) {
  ...
  if (taskUpdateDB.parent?.taskId) {
    ...
    await helper(
      ...
      req.organisationId,
      req.userId,
      new Date(dependentTaskStartDate),
      false
    );
  }
  ...
  for (const key in taskUpdateValue) {
    if (updatedValueWithoutOtherTable[key] !== findTaskWithoutOtherTable[key]) {
      ...
      const historyData = {
        oldValue:
          key === "completionPecentage" && !findTaskWithoutOtherTable[key]
            ? 0
  ...
  let finalResponse: Task | any = { ...returnUpdatedTask, endDate };

  if (returnUpdatedTask.parent) {
    const parentEndDate = await taskEndDate(
...
export const statusChangeTask = async (
  ...
  if (
    statusBody.status === TaskStatusEnum.IN_PROGRESS ||
      ...
      if (obj.dependentType === TaskDependenciesEnum.PREDECESSORS) {
        ...
        if (
          ...
          throw new BadRequestError(
            `You can not change the status for this task since 
...
export const statusCompletedAllTAsk = async (
  req: express.Request,
  res: express.Response
) => {
  if (!req.userId) {
  ...
  if (findAllTaskByProjectId.length > 0) {
    ...
    return new SuccessResponse(
      StatusCodes.OK,
      null,
      "all task status change to completed successfully"
    ).send(res);
  }
};
...
export const addComment = async (
  ...
  const taskId = uuidSchema.parse(req.params.taskId);
  const { commentText, parentCommentId } = createCommentTaskSchema.parse(
    req.body
  );
  const prisma = await getClientByTenantId(req.tenantId);
...
export const updateComment = async (
  ...
  const prisma = await getClientByTenantId(req.tenantId);
  const { hasAccessIf, findComment } = await commentEditorDelete(
    commentId,
    req.role!,
    req.userId,
    req.tenantId
...
export const addAttachment = async (
  ...
  const prisma = await getClientByTenantId(req.tenantId);
  const { hasAccessIf, findAttchment } = await attachmentAddOrRemove(
    taskId,
    req.role!,
    req.userId,
    req.tenantId,
...
export const deleteMemberFromTask = async (
  ...
  if (!hasAccessIf) {
    throw new UnAuthorizedError(
      "You are not authorized to remove member from task"
    );
  }
...
export const addDependencies = async (
  ...
  const taskId = uuidSchema.parse(req.params.taskId);
  const prisma = await getClientByTenantId(req.tenantId);
  const { hasAccessIf, dependencies, findTask } = await dependenciesAddOrRemove(
    taskId,
    req.role!,
    req.userId,
    req.tenantId,
  ...
  if (
    ...
    throw new BadRequestError(
      "You can not designate an ongoing or completed task as a successor of an incompleted task. You should set the progress percentage to 0% to be able to do it"
    );
  }
  ...
  const [addDependencies1, addDependencies2] = await addDependenciesHelper(
    taskId,
    dependendentOnTaskId,
    req.tenantId,
    req.organisationId,
  ...
  await prisma.history.createHistory(
    ...
    historyMessage1,
    historyData1,
    addDependencies1.dependentTaskId
  );
  ...
  await prisma.history.createHistory(
    ...
    historyMessage2,
    historyData2,
    addDependencies1.dependendentOnTaskId
  );
  return new SuccessResponse(
    StatusCodes.OK,
    null,
...
export const removeDependencies = async (
  ...
  const prisma = await getClientByTenantId(req.tenantId);
  const { hasAccessIf, dependencies } = await dependenciesAddOrRemove(
    taskDependenciesId,
    req.role!,
    req.userId,
    req.tenantId,
  ...
  if (!hasAccessIf) {
    throw new UnAuthorizedError(
      "You are not authorized to remove dependencies"
    );
  }
...
export const addOrRemoveMilesstone = async (
  ...
  if (!hasAccessIf) {
    throw new UnAuthorizedError(
      "You are not authorized to add or remove milestone for this task"
    );
  }
  ...
  const milestone = await prisma.task.update({
    data: {
      ...
      parentTaskId:
        milestoneIndicator && findtask.parentTaskId
          ? null
  ...
  if (milestone && milestone.parent?.taskId) {
    ...
    const subtaskDurations =
      updatedParent?.subtasks.map((subtask) => subtask.duration) ?? [];
...
export const reAssignTaskToOtherUser = async (
  req: express.Request,
  res: express.Response
) => {
...
export const allTaskOfUser = async (
  req: express.Request,
  res: express.Response
) => {
  const userId = req.userId;
  ...
  return new SuccessResponse(
    StatusCodes.OK,
    finalArray,
    "get users task successfully"
  ).send(res);
};

export const createTaskIfNotExists = async (
  req: express.Request,
  res: express.Response
) => {
  const userId = req.userId;
...
export const selectedDeleteTask = async (
  req: express.Request,
  res: express.Response
) => {
  const prisma = await getClientByTenantId(req.tenantId);
...
export const addMemberToMultiTask = async (
  req: express.Request,
  res: express.Response
) => {
  if (!req.userId) {
  ...
  return new SuccessResponse(
    StatusCodes.CREATED,
    member,
    "Member added to selected task successfully"
  ).send(res);
};
...
export const duplicateTask = async (
  ...
  const generateUniqueTaskName = async (name: string) => {
    let newName = name;
    let counter = 1;
    while (true) {
          ...
          await Promise.all(
            secondsubtask.subtasks.map(async (thirdSubTask) => {
              ...
              if (thirdSubTaskInsert && thirdSubTask.subtasks.length > 0) {
                await Promise.all(
                  thirdSubTask.subtasks.map(async (forthSubTask) => {
                    const {
                      taskId,
                      documentAttachments,
                      status,
