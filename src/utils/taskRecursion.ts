import { ProjectStatusEnum, Task, TaskStatusEnum } from "@prisma/client";
...
export const timeline = async (
  ...
  if (task.subtasks.length > 0) {
    for (const subtask of task.subtasks) {
      const endDate = await calculateEndDate(
        subtask.startDate,
        subtask.duration,
        tenantId,
        organisationId
...
export const updateSubtasksDependencies = async (
  taskId: string,
  endDate: Date,
  userId: string,
  tenantId: string
